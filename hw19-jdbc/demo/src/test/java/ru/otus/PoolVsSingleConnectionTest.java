...
@DisplayName("Comparing the speed of work with connectionPool and without it ")
@Testcontainers
class PoolVsSingleConnectionTest {
    ...
    @ParameterizedTest(name = " using connection Pool: {0}")
    ...
    void doSelect(boolean usePool) throws SQLException {
        ...
        try (Connection connection = getConnection(usePool)) {
            logger.info("getting connection [{}]",connection.getClientInfo());
        }
